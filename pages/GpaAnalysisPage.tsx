    ...
    if (active && payload && payload.length) {
        return (
            <div className="bg-bg-panel dark:bg-dark-panel p-2 border border-slate-300 dark:border-dark-border rounded-lg shadow-sm">
                <p className="font-semibold text-text-primary dark:text-dark-text-primary">{`GPA Range: ${label}`}</p>
                <p style={{ color: payload[0].fill }}>{`Students: ${payload[0].value}`}</p>
            </div>
        );
    }
    return null;
...
interface GpaAnalysisPageProps {
    allStudents: AnalyzedStudent[];
}

const GpaAnalysisPage: React.FC<GpaAnalysisPageProps> = ({ allStudents }) => {
    ...
    const { gpaStats, gpaDistributionData } = useMemo(() => {
        ...
        const stats = {
            avg: gpas.reduce((a, b) => a + b, 0) / gpas.length,
            max: Math.max(...gpas),
            min: Math.min(...gpas),
        };

        const distribution = [
            { range: '0.0-0.99', count: 0 },
            { range: '1.0-1.99', count: 0 },
            { range: '2.0-2.99', count: 0 },
            { range: '3.0-4.0', count: 0 },
        ];
        gpas.forEach(gpa => {
            if (gpa < 1) distribution[0].count++;
            else if (gpa < 2) distribution[1].count++;
            else if (gpa < 3) distribution[2].count++;
    ...
    const Row = ({ index, style }: { index: number, style: React.CSSProperties }) => {
        ...
        return (
            <div style={style} className={`flex items-center text-sm p-2 border-b border-slate-100 dark:border-dark-panel-hover ${index % 2 === 0 ? 'bg-slate-50 dark:bg-dark-panel/50' : ''}`}>
                <div className="w-2/5 font-semibold text-text-primary dark:text-dark-text-primary">{student.fullName} ({student.navaId})</div>
                <div className="w-1/5 text-text-secondary dark:text-dark-text-secondary">{student.techGroup}</div>
                <div className="w-1/5 text-text-secondary dark:text-dark-text-secondary">{student.company}</div>
                <div className="w-1/5 font-bold text-lg text-brand-primary dark:text-brand-primary">{student.gpa?.toFixed(2) ?? 'N/A'}</div>
            </div>
        );
    };
    ...
    return (
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            <div className="lg:col-span-2 flex flex-col gap-6">
                <ChartContainer title={`Student GPA Rankings (${sortedStudents.length})`}>
                    <div className="flex flex-col h-full">
                        <div className="flex-shrink-0 flex p-2 bg-slate-100 dark:bg-dark-panel-hover font-semibold text-sm text-text-secondary dark:text-dark-text-secondary sticky top-0 z-10">
                            <button className="w-2/5 text-left" onClick={() => handleSort('fullName')}>Student Name <SortIcon direction={sortConfig.key === 'fullName' ? sortConfig.direction : undefined} /></button>
                            <button className="w-1/5 text-left" onClick={() => handleSort('techGroup')}>Tech Group <SortIcon direction={sortConfig.key === 'techGroup' ? sortConfig.direction : undefined} /></button>
                            <button className="w-1/5 text-left" onClick={() => handleSort('company')}>Company <SortIcon direction={sortConfig.key === 'company' ? sortConfig.direction : undefined} /></button>
                            <button className="w-1/5 text-left" onClick={() => handleSort('gpa')}>GPA <SortIcon direction={sortConfig.key === 'gpa' ? sortConfig.direction : undefined} /></button>
                        </div>
                        <div className="flex-grow">
                            <AutoSizer>
                                {({ height, width }) => (
                                    <List height={height} width={width} itemCount={sortedStudents.length} itemSize={50}>
                                        {Row}
                                    </List>
                                )}
                            </AutoSizer>
            ...
            <div className="lg:col-span-1 space-y-6">
                <div className="grid grid-cols-1 sm:grid-cols-3 lg:grid-cols-1 gap-4">
                     <KpiCard title="Average GPA" value={gpaStats.avg.toFixed(2)} />
                     <KpiCard title="Highest GPA" value={gpaStats.max.toFixed(2)} />
                     <KpiCard title="Lowest GPA" value={gpaStats.min.toFixed(2)} />
                </div>
                <ChartContainer title="GPA Distribution">
                    <ResponsiveContainer width="100%" height="100%">
                        <BarChart data={gpaDistributionData} margin={{ top: 5, right: 20, left: -10, bottom: 5 }}>
                            <CartesianGrid strokeDasharray="3 3" />
                            <XAxis dataKey="range" />
                            <YAxis allowDecimals={false} />
                            <Tooltip content={<CustomTooltip />} />
                            <Bar dataKey="count" name="Students">
                                {gpaDistributionData.map((entry, index) => <Cell key={`cell-${index}`} fill={['#ef4444', '#f59e0b', '#62B766', '#10b981'][index]} />)}
                            </Bar>
                        </BarChart>
                    </ResponsiveContainer>
                </ChartContainer>
