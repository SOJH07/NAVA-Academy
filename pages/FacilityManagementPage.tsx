...
const categoryStyles = {
    ...
    'other': 'bg-fp-other-bg text-fp-other-text border border-fp-other-border',
};

const FloorCard: React.FC<{ floor: Floor }> = ({ floor }) => (
    <div className="bg-white rounded-xl shadow-lg overflow-hidden flex flex-col">
        <h3 className="text-2xl font-bold text-text-primary p-4 bg-slate-50 text-center">{floor.name}</h3>
            ...
            {floor.layout.map((row, rowIndex) => (
                <React.Fragment key={rowIndex}>
                    {row.map((cell, cellIndex) => (
                        <div 
                            key={cellIndex} 
                            className={`flex items-center justify-center text-center p-2 border-t border-slate-200 h-24 text-xs font-semibold whitespace-pre-wrap text-wrap-balance ${categoryStyles[cell.category]} ${cellIndex === 0 ? 'border-r' : ''}`}
                        >
                            {cell.text}
                        </div>
                    ))}
    ...
    return (
        <div className="space-y-8 animate-fade-in">
            ...
            <div className="bg-white rounded-xl shadow-lg p-6">
                <h3 className="text-2xl font-bold text-text-primary mb-6 text-center">Legend & Floor Guide</h3>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-6 mb-6">
                    ...
                    <LegendItem colorClass="bg-fp-ev-industrial-bg" label="EV-Industrial (Violet)" description="LAP / WS – workshops, DPIT – theory classes" />
                ...
                <div className="border-t border-slate-200 pt-4">
                     <p className="text-sm text-text-secondary"><strong className="text-text-primary">Floor Guide:</strong> 3rd Floor – mostly classes; 2nd Floor – all classes; 1st Floor – all LAP / WS; Ground Floor – workshops.</p>
                </div>
            </div>
        </div>
    );
