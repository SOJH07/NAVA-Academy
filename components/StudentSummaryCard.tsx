...
const StudentSummaryCard: React.FC<StudentSummaryCardProps> = ({ student, onClick }) => {
    ...
    return (
        <button 
            onClick={onClick}
            className={`w-full h-full text-left bg-bg-panel dark:bg-dark-panel border ${companyColor} rounded-xl shadow-md p-4 flex flex-col justify-between transition-all duration-300 hover:shadow-lg hover:ring-2 hover:ring-brand-primary overflow-hidden`}>
            ...
            {/* Bottom part */}
            <div className="mt-3 pt-3 border-t border-slate-200 dark:border-dark-border flex justify-between items-center text-sm">
                <div>
                    <span className="font-semibold text-text-secondary dark:text-dark-text-secondary">GPA: </span>
                    <span className="font-bold text-lg text-brand-primary dark:text-brand-primary">{student.gpa?.toFixed(2) ?? 'N/A'}</span>
                </div>
                {student.aptisScores && (
                     <div>
                        <span className="font-semibold text-text-secondary dark:text-dark-text-secondary">APTIS: </span>
