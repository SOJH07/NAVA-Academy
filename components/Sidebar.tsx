...
const Sidebar: React.FC<SidebarProps> = ({ pages, activePage, setActivePage, onLogout }) => {
  ...
  return (
    <aside className={`bg-[#1E361E] border-r border-[#2A4B2A] p-4 flex flex-col flex-shrink-0 transition-all duration-300 ease-in-out ${isSidebarCollapsed ? 'w-20' : 'w-64'}`}>
      {/* Header */}
            ...
            <li key={page.id} className="mb-2">
              <button
                ...
                className={`w-full text-left flex items-center gap-3 px-4 py-2 rounded-lg transition-colors duration-200 ${isSidebarCollapsed ? 'justify-center' : ''} ${
                  activePage === page.id
                    ? 'bg-brand-primary-dark text-white font-semibold'
                    : 'text-dark-text-secondary hover:bg-dark-panel hover:text-dark-text-primary'
                }`}
              >
      ...
      <div className="flex-shrink-0">
        <div className="border-t border-dark-border pt-2 space-y-1">
            <button 
                onClick={onLogout}
                className={`w-full text-left flex items-center gap-3 px-4 py-2 rounded-lg transition-colors duration-200 text-dark-text-muted hover:bg-dark-panel hover:text-dark-text-primary ${isSidebarCollapsed ? 'justify-center' : ''}`}
                title="Logout"
            >
                 <div className="flex-shrink-0">
        ...
        <div className={`mt-2 text-center text-xs text-dark-text-muted whitespace-nowrap transition-opacity ${isSidebarCollapsed ? 'opacity-0 hidden' : 'opacity-100'}`}>
