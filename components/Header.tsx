...
const SearchIcon = () => <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5 text-text-muted dark:text-dark-text-muted" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}><path strokeLinecap="round" strokeLinejoin="round" d="M21 21l-5.197-5.197m0 0A7.5 7.5 0 105.196 5.196a7.5 7.5 0 0010.607 10.607z" /></svg>;
...
const Header: React.FC<HeaderProps> = ({ pageTitle, pageIcon, onFilterButtonClick, activeFilterCount, globalSearchTerm, onSearchChange }) => {
    return (
        <header className="px-6 pt-6 pb-4 bg-bg-body dark:bg-dark-body z-10 flex justify-between items-center gap-4">
            <div className="flex items-center gap-4">
                ...
                <h1 className="text-3xl font-extrabold text-text-primary dark:text-dark-text-primary tracking-tight">{pageTitle}</h1>
            </div>
            <div className="flex items-center gap-3">
                 <div className="relative">
                    ...
                    <input 
                        ...
                        className="bg-bg-panel dark:bg-dark-panel border border-slate-300 dark:border-dark-border rounded-lg py-2 px-4 pl-10 text-text-primary dark:text-dark-text-primary focus:outline-none focus:ring-2 focus:ring-brand-primary w-full md:w-80 shadow-sm"
                    />
                </div>
                <button
                    onClick={onFilterButtonClick}
                    className="flex items-center gap-2 px-4 py-2 bg-bg-panel dark:bg-dark-panel border border-slate-300 dark:border-dark-border rounded-lg shadow-sm text-text-primary dark:text-dark-text-primary font-semibold hover:bg-bg-panel-hover dark:hover:bg-dark-panel-hover transition-colors"
                >
                    <FilterIcon />
                    <span>Filters</span>
                    {activeFilterCount > 0 && (
